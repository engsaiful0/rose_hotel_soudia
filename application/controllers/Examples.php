...
class Examples extends CI_Controller
{
    ...
    public function add_late_iframe()
    {
        ...
        {    $crud->display_as('amount', 'مقدار');
            $crud->display_as('date', 'تاريخ');

            $crud->display_as('hotel_id', 'الفندق');
            $crud->display_as('remarks', 'ملاحظات');
        }
...
//            die('3');
    ...
    public function add_room_iframe()
    {
        ...
        {
            $crud->display_as('room_no_in_english', 'الغرفة لا باللغة الإنجليزية');
            $crud->display_as('hotel_id', 'الفندق');
    ...
    public function add_hotel_iframe()
    {
        ...
        {
            $crud->display_as('hotel_name_in_english', 'اسم الفندق بالانجليزية');
            $crud->display_as('hotel_name_in_arabic', 'اسم الفندق بالعربي');
            $crud->display_as('address_in_english', 'العنوان باللغة الإنجليزية');
            $crud->display_as('address_in_arabic', 'العنوان بالعربية');
        }
...
public function print_text()
    {
        ...
        $data['output_content'] = 'examples/print_text_iframe';
        $data['flag'] = 'admin';
        $this->load->view('backend/print_content', $data);
    }
    public function print_text_iframe()
    {
        $crud = new Grocery_crud();
        $crud->unset_add();
        $crud->unset_delete();
        $crud->set_table('print_text');
        $language = $this->session->userdata('language');
        if ($language == 'english') {
            $crud->set_language('english');
            $crud->set_subject('Print Text');
        }else
        {$crud->set_language('arabic');
            $crud->set_subject('طباعة نص');
        }
       
        $crud->required_fields('print_text_description');
        $crud->columns('print_text_description');
        $crud->fields('print_text_description');
        $crud->display_as('print_text_description', 'Print Text');
        $output = $crud->render();
        $data = array();
        $data['flag'] = '';
        $this->load->view('backend/member_show_to_admin.php', $output);
