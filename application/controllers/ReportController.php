...
class ReportController extends CI_Controller
{
    ...
    public function view_purchase()
    {
        ...
        $result["result"] = $this->pagmodel->get_purchase($config["per_page"], $page);
    ...
    {
        $data['output_content'] = $this->load->view('report/transaction_report', '', true);
        $data['flag'] = '';
        $this->load->view('admin_content', $data);
    }

    public function transaction_report_load()
    {
        $data = array();
        $from_date = '';
        $to_date = '';
        ...
        $this->load->view('report/transaction_report_load', $data);
    }
    public function transaction_report_load_pdf($ids)
    {
        $ids = explode("_", $ids);
        //        echo '<pre>';
        //        var_dump($ids);
        ...
        $this->pdf->load_view('report/transaction_report_load_pdf', $data);

        $this->pdf->render();
        if ($ids[0] != '' && $ids[1] != '' && $ids[2] != '') {
            $hotel = $this->db->select('*')
                ->where('hotel_id', $ids[2])
                ->get('hotel')->row();
            $this->pdf->stream('Transaction Report For <b>' . $hotel->hotel_name_in_english . '</b> From Date ' .  dateRangeToText($ids[0], $ids[1]) . '.pdf');
        } else if ($ids[0] != '' && $ids[1] == '' && $ids[2] == '') {

            $this->pdf->stream('Transaction Report of Date ' . date('d-m-Y', strtotime($ids[0])) . '.pdf');
        } else if ($ids[0] == '' && $ids[1] != '' && $ids[2] == '') {

            $this->pdf->stream('Transaction Report of Date ' . date('d-m-Y', strtotime($ids[1])) . '.pdf');
        } else if ($ids[0] == '' && $ids[1] == '' && $ids[2] != '') {
            $hotel = $this->db->select('*')
                ->where('hotel_id', $ids[2])
                ->get('hotel')->row();
            $this->pdf->stream('Transaction Report of <b>' . $hotel->hotel_name_in_english . '</b>.pdf');
        } else if ($ids[0] == '' && $ids[1] == '' && $ids[2] == '') {
            $this->pdf->stream('Transaction Report.pdf');
        }
    }
    public function dateRangeToText($from_date, $to_date)
    {
