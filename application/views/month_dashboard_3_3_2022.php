    ...
    <div class="card-body">
        <div class="row">
            <div class="col-md-12">
                <?php
                $sl = 1;
                $rooms = $this->db->where('status', 'booked')->where('day_or_month', 'month')->where('hotel_id', $hotel->hotel_id)->get('room')->result();
                foreach ($rooms as $room) {
                    $checkin_details_id_for_renew = 0;
                    ?>
                    <div class="container-room"
                         style="width: 10%;float: left;margin-right: 5px;padding-top: 10px;background-color: #00C853;margin-top: 5px;  ">
                        <div>
                        ...
                        <div>
                            <input style="width: 100%" data-toggle="modal"
                                   data-target="#exampleModal<?php echo $room->checkin_id ?>"
                                   type="button"
                                   class="btn-primary btn large" value="<?php
                            if ($language == 'english') {
                                echo $room->price;
                            } else {
                                                ...
                                                foreach ($checkin_details as $checkin_detail) {
                                                    ...
                                                    if ( $data>=$checkin_detail->dateOfExit) {
                                                        if (($time >= 15 && $min > 0 && $checkin_detail->exit_status == 'no')) {
                                                            ...
                                                            $this->db->where('room_id', $checkin_detail->room_id)->update('room', $data_room);
